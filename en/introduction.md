...
* Feeds should provide a plain text output in addition to the protocol-buffer encoded feed content (e.g., by appending `&debug` to the feed URL). This allows for easier viewing of feed contents for troubleshooting purposes without requiring special tools to simply view the current contents of the real-time feed.
