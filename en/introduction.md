...
* Feeds should provide an option to display data in a structured UTF-8 text output (e.g., JSON, Protobuf [TextFormat](https://developers.google.com/protocol-buffers/docs/reference/java/com/google/protobuf/TextFormat)) in addition to the protocol-buffer encoded feed content (e.g., by appending `&debug` to the feed URL, providing an alternate URL to different file such as `trip-updates.json`). This enables a simple human-readable version of the information for troubleshooting purposes without requiring special tools to view the current contents of the real-time feed.
