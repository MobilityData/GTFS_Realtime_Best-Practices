...
* Feeds should provide an option to display data in a structured UTF-8 text output (e.g., JSON) in addition to the protocol-buffer encoded feed content (e.g., by appending `&debug` to the feed URL, providing an alternate URL to different file such as `trip-updates.json`). This allows for easier viewing of feed contents for troubleshooting purposes without requiring special tools to simply view the current contents of the real-time feed.
